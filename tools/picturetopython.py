...
cases = [
    "yppggokoobbbyykpkkkgkkkkkk"
]

# Replace polygon fills sequentially with hex codes
def apply_colors(svg_str, colors, mapping):
    # Convert names to hex codes
    hex_colors = [mapping[c] for c in colors]
    
    polygons = re.findall(r"<polygon fill=['\"]#\w{3,6}['\"]", svg_str)
    if len(polygons) != len(hex_colors):
        raise ValueError(f"Number of polygons ({len(polygons)}) != number of colors ({len(hex_colors)})")
    
    def replacer(match):
        # Pop the first hex color
        color = hex_colors.pop(0)
        return f"<polygon fill='{color}'"
    
    return re.sub(r"<polygon fill=['\"]#\w{3,6}['\"]", replacer, svg_str)

# Extract the base SVG template
svg_template = list(data.values())[0]

def make_svg(cases, offset):

    result = {}

    for i, sticker_string in enumerate(cases):
        key = str(i + offset)
        
        # Convert sticker string into list of characters
        sticker_colors = list(sticker_string)
        
        # Apply coloring
        colored_svg = apply_colors(svg_template, sticker_colors.copy(), color_to_hex)
        
        result[key] = colored_svg
